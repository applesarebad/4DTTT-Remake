...
export function d1Store(db) {
    return {
         ...
         */
        async record({ day, clientId, outcome, moves, at }) {
            ...
            await db
                .prepare(
                    `INSERT INTO attempts (day, client_id, outcome, moves, first_seen, solved_at)
                     VALUES (?1, ?2, ?3, ?4, ?5, ?6)
                ...
                .bind(day, clientId, outcome, moves, at, solvedAt)
                .run();
        ...
        async totals(day) {
            const row = await db
                ...
                .bind(day)
                .first();
            return { attempts: row?.attempts ?? 0, solved: row?.solved ?? 0 };
        },
    };
}
